...
int main()
{
	...
	if (year < YEAR0 || year > 9999 || month < 1 || month > 12 || day < 1 || day > 31 || hour < 0 || hour > 23 || minute < 0 || minute > 59 || second < 0 || second > 59) // 加入了对时间输入是否合法的判断
	{
		printf("输入的时间不合法！\n");
		return 1;
	}
	...
	else
	{
		...
		totalSeconds = countYears(year) + countMonths(year, month) + (day - 1) * 24 * 3600 + hour * 3600 + minute * 60 + second; // 计算总秒数
		printf("%lld", totalSeconds);
		return 0;
	}
}
//--------------------------------------------------------------------
